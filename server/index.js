...
function buildSystemPrompt({ us = false, domain = 'umum' } = {}) {
...
app.post('/api/translate', async (req, res) => {
  try {
    ...
    const raw =
      PROVIDER === 'groq'
        ? await callGroq({ term: term.trim(), system })
        : await callAnthropic({ term: term.trim(), system });
...
app.listen(PORT, () => {
  console.log(`API proxy listening on http://localhost:${PORT} (provider: ${PROVIDER})`);
});
